...
import 'package:zilli/Model/all_adnetworks.dart';
import 'package:zilli/Model/history/check_quiz_model.dart';
...
class RewardRepo {
  ...
  Future<CheckQuizModel> checkQuiz(String quizId) async {
    ...
    var response = await http.post(url, headers: {'Authorization': 'Bearer $token'}, body: <String, dynamic>{'quiz_id': quizId,});
  ...
  Future<bool> addQuizPoint(String amount, String quizId, String winStatus, String resultStatus) async {
    ...
    var response = await http.post(url, headers: {'Authorization': 'Bearer $token'}, body: <String, dynamic>{'coin': amount, 'quiz_id': quizId,'win_status': winStatus,'result_status': resultStatus,});
    if (response.statusCode == 200) {
      return true;
    } else {
      throw Exception('An error occurred. Please try again');
